<template>
  <div class="date">
    ...
    <div v-if="isOpenedMenu" class="date-menu">
      <div class="date-menu-wrapper">
        <div
          ...
          @click="onClickChangePeriodType(datePeriod.name)"
        >
          {{ datePeriod.title }}
        </div>
      </div>
      ...
      <div class="date-menu-wrapper">
        <div
          ...
          @click="onClickSelectLastThirtyDays"
        ...
        <div
          ...
          @click="onClickChangePeriodType('ownPeriod')"
        >
      ...
      <div v-if="isShownDateMenu">
        <div v-if="isShownOwnRange">
          ...
          >
            <template #day="{ dayFormatted, isInRange, isFirstDayOfRange, isToday }">
              <span v-if="isToday" class="own-today"> {{ dayFormatted }}</span>

              <span v-else-if="isInRange" class="own-range-day">{{ dayFormatted }}</span>

              <span v-else-if="isFirstDayOfRange" class="first-own-range-day">{{
                dayFormatted
              }}</span>
        ...
        <div v-else>
          ...
          <div class="date-menu-list" :class="menuTypeClass">
            <div
              ...
              @click="onClickSelectValue(index)"
            >
              {{ date.title }}
            </div>
          </div>
  ...
  computed: {
    ...
    ownRangeClasses() {
      return {
        ...
        navigatorViewButton: `order-1 w-full justify-center cursor-pointer text-sm font-medium text-base-dark`,
        ...
        day: `text-sm font-medium text-base-dark rounded-lg w-full h-10 mx-auto hover:bg-gray-100 disabled:opacity-50
          disabled:cursor-not-allowed`,
...
.first-own-range-day {
  @apply flex items-center justify-center;
  @apply h-full rounded-l-lg bg-base-dark bg-opacity-10 font-medium text-base-dark;

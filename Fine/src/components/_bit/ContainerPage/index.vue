...
export default {
  ...
  computed: {
    ...
    pageBgImage() {
      return this.bgImage && this.bgImagePath && !this.isMobileDevice
        ? `background-image: url(${this.bgImagePath});`
        : "";
    },

    pageClasses() {
      const theme = getTheme();
      let bgClass = "bg-base-light bg-fixed bg-center";
      const minHeight = this.isMobileDevice ? "min-height-mobile" : "min-height-desktop";

      if (this.darkMode) {
        bgClass = "bg-base-dark";
      }

      return [theme, bgClass, minHeight];
    },
